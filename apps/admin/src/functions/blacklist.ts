...
const $listBlacklist = createServerFn({ method: "GET" })
  ...
  .handler(({ context }) => listBlacklist(context.core.db));

export const blacklistQueryOptions = () =>
  queryOptions({
    queryKey: queryKeys.blacklist,
    queryFn: ({ signal }) => $listBlacklist({ signal }),
  });

export type BlacklistRow = Awaited<ReturnType<typeof $listBlacklist>>[number];
